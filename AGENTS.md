...
The committed `skills/chrome-devtools-axi/SKILL.md` is generated by `pnpm run build:skill`; `pnpm test` fails if it drifts from `createSkillMarkdown()`. The shipped skill stays a minimal stub and defers to the CLI for all actual guidance (chrome-devtools-axi CLI output is the single source of truth); never re-duplicate CLI-owned instructions into the skill. Prefer a pointer over restated detail.
