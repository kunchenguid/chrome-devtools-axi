...
`closepage` never trusts a numeric id by itself. An explicit `pages` command returns a one-time token and stores it with a per-id URL digest (never raw URLs) in the session state directory; `closepage` requires and atomically consumes that token, re-lists, and closes only if the target id still shows its observed URL, so unrelated tabs may change meanwhile. One listing authorizes one close, so concurrent callers cannot borrow another listing, and a reconnect or another compatible MCP implementation that reissues ids cannot silently retarget a stale id. The re-list is the guard against page-initiated changes (popups, `window.close()`, navigation); AXI's own `new_page`/`close_page`, missing-page errors, respawn, `stop`, and browser reconnects additionally clear any unconsumed observation. Identity is id plus URL, so two tabs on the same URL are indistinguishable.
