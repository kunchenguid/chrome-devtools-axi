...
Because `ensureBridge` deep-probes first, that probe usually consumes the marker instead, so `/health?deep=1` relays it as `pageIdentityChanged` - only when the clear actually removed an id, so a never-selected session is never told it lost routing - and `checkBridgeHealth` records it for the invoking process (`takePageIdentityChangeNotice`); a page-scoped call in that same process then names the reconnect instead of the plain no-selection error, while a process that makes none (`pages`, the home view probe) just drops the notice.
