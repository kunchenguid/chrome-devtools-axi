...
chrome-devtools-mcp 1.8 requires an explicit `pageId` on page-scoped tools (`click`, `fill`, `take_snapshot`, `evaluate_script`, ...), while the CLI keeps page selection as ambient state across its short-lived processes. `callTool` (`src/client.ts`) closes that gap: for any tool in `PAGE_SCOPED_TOOLS` without a caller-supplied `pageId`, it resolves the currently selected page via `list_pages` and forwards it explicitly, so the call is independent of a concurrent selection change in the same session. Browser-scoped tools (`list_pages`, `new_page`, `select_page`, `close_page`) are intentionally excluded since they establish or manage that selection themselves.
