...
chrome-devtools-mcp 1.8+ requires `pageId` on page-scoped tools by default. `callTool` (`src/client.ts`) injects the session's last AXI `select_page` id (`src/selected-page.ts`); `list_pages` is display-only and never sets routing. `new_page` records the unique complete row in its own dump whose URL matches `args.url` (`about:blank` plus the requested URL is fine); title continuations, zero matches, and two matching URLs leave the id unset so the next page-scoped call fails loud until `select_page`. The `pages` selected column overlays `getSelectedPageId()`, not MCP `[selected]`. Missing selection fails loudly with "No page is currently selected". `close_page` of the selected id clears it. chrome-devtools-mcp deliberately reissues all page ids after an in-process browser reconnect, so `didMcpPageIdentityChange` (`src/bridge.ts`) observes its one-shot marker on the raw MCP result - on `/call` responses and on the deep health probe - before the output is flattened to text, and clears the persisted selection; because the reconnect happened _during_ that call, a `/call` that carried an explicit `pageId` fails with `PAGE_IDENTITY_CHANGED_ERROR` (`src/bridge-script.ts`) ahead of the tool-error branch - upstream hands out page ids from a monotonic counter, so the real reconnect path is an `isError` body naming a missing page, and the caller must be told to re-select rather than hunt a closed tab - while a call that named no page (the home view probe) still renders. A race or ordinary target close that reaches MCP as `No page found` is translated to an actionable `BROWSER_ERROR` and clears that same selected id rather than retargeting another tab. Both dependency-owned sentences are matched positionally, matching upstream's emission order: the reconnect notice only on the first line of the flattened body, `No page found` / `The selected page has been closed.` only on its last non-empty line. Page-owned strings upstream interpolates verbatim (a dialog message, a title) can contain raw newlines, so any line in between is forgeable and a whole-body or any-line match is not. That emission order is an _unverified dependency contract_ - chrome-devtools-mcp is spawned via npx rather than installed as a devDependency, so no test pins it. If upstream reorders, the observable effect is a less accurate message, not a silent retarget: the two matchers are independent nets and upstream issues page ids from a process-wide monotonic counter, so a stale id fails to resolve rather than landing on an unrelated page. The clean boundary for both matchers is upstream's `structuredContent` - already preferred for the reconnect signal when present, still behind an experimental flag - and once upstream stabilizes it both text matchers become redundant.
