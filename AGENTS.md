...
Because `ensureBridge` deep-probes first, that probe usually consumes the marker instead, so `/health?deep=1` relays it as `pageIdentityChanged` - only when the clear actually removed an id, so a never-selected session is never told it lost routing - and `checkBridgeHealth` records it for the invoking process (`takePageIdentityChangeNotice`); a page-scoped call in that same process then names the reconnect instead of the plain no-selection error, while a process that resolves no selection just drops it - `pages` only calls `list_pages`, and the home view probe records no notice at all because its own health check is shallow.
It is one-shot (each deep probe overwrites it, reading consumes it), and a respawn stays plain because `ensureBridge` clears the selection before spawning, leaving the new bridge's first probe nothing to drop.
