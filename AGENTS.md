...
Because `ensureBridge` deep-probes first, that probe usually consumes the marker instead, so `/health?deep=1` relays it as `pageIdentityChanged` and `checkBridgeHealth` records it for the invoking process (`takePageIdentityChangeNotice`); the next page-scoped call then names the reconnect rather than reporting the plain no-selection error, and the notice is one-shot (each deep probe overwrites it, reading consumes it) so a never-selected or freshly respawned session keeps the plain message.
