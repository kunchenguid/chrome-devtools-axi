...
`CHROME_DEVTOOLS_AXI_MCP_SERVER_URL` is forwarded to that child as `CHROME_DEVTOOLS_MCP_SERVER_URL`, selecting chrome-devtools-mcp's stdio-to-Streamable-HTTP proxy mode. Each named AXI bridge therefore keeps a separate remote MCP context while sharing the remote service's Chrome. Without it, connection modes are env-driven through `buildTransportArgs`: `AUTO_CONNECT` (Chrome 144+ remote debugging), `BROWSER_URL` (http(s) -> `--browserUrl`, ws(s) -> `--wsEndpoint` + `WS_HEADERS`), `USER_DATA_DIR` (persistent profile) vs the default `--isolated`, `CHANNEL` (`--channel` to pick which installed Chrome release channel is attached to or launched, omitted in `BROWSER_URL`/`wsEndpoint` mode), and `HEADED`.
