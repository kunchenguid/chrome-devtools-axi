...
`chrome-devtools-axi sessions` (`src/doctor.ts`) inventories default, named, and pooled PID state without starting a browser; cleanup remains opt-in and validates PID death or bridge ownership before removing state or signaling a process.
