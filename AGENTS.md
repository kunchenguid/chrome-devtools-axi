...
`chrome-devtools-axi sessions` (`src/doctor.ts`) inventories default, named, and pooled PID state without starting a browser; cleanup remains opt-in and delegates live physical stops to the `stopBridgeSession` identity invariant.
