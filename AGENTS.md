...
`closepage` never trusts a numeric id by itself. An explicit `pages` command returns a one-time token and stores it with a digest (never raw URLs) of the complete id/url list in the session state directory; `closepage` requires and atomically consumes that token, re-lists, and closes only if the whole list is unchanged. One listing authorizes one close, so concurrent callers cannot borrow another listing and a close that reindexes the remaining tabs cannot make a second stale id target another tab. Browser reconnects and page mutations clear any unconsumed observation.
