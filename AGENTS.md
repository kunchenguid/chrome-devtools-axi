...
`CHROME_DEVTOOLS_AXI_STATE_DIR` overrides the _base_ state directory (`resolveStateBaseDir` in `src/sessions.ts`) that every state path is derived from - the pidfile, the `snapshot-generation` counter, and any per-name session subdir all move under it - so an independent caller can own an isolated `bridge.pid` without a named session; pair it with a distinct `CHROME_DEVTOOLS_AXI_PORT` because two bridges still cannot share one TCP listener.
