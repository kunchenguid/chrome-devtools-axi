...
The no-mistakes agent skill is supplied from the user's skill installation, not vendored in this repository; keep `.agents/skills/` present only as the repository-local `.claude/skills` symlink target.
...
- Keep `.agents/skills/` present as the repository-local `.claude/skills` symlink target, but do not vendor no-mistakes there; contributors should use their user-level no-mistakes skill installation.
