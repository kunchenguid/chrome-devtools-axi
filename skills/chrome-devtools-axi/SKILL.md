...
You do not need chrome-devtools-axi installed globally - invoke it with `CHROME_DEVTOOLS_AXI_IDLE_TIMEOUT_MS=120000 npx -y chrome-devtools-axi <command>`.
The idle timeout prefix keeps agent-started headless Chrome sessions bounded if an agent exits without running `stop`.
If chrome-devtools-axi output shows a follow-up command starting with `chrome-devtools-axi`, run it as `CHROME_DEVTOOLS_AXI_IDLE_TIMEOUT_MS=120000 npx -y chrome-devtools-axi ...` instead.
...
1. Run `CHROME_DEVTOOLS_AXI_IDLE_TIMEOUT_MS=120000 npx -y chrome-devtools-axi open <url>` to navigate. Output includes the page's accessibility snapshot; interactive elements carry `uid=` refs.
...
Run `CHROME_DEVTOOLS_AXI_IDLE_TIMEOUT_MS=120000 npx -y chrome-devtools-axi --help` for flags and environment variables, or `CHROME_DEVTOOLS_AXI_IDLE_TIMEOUT_MS=120000 npx -y chrome-devtools-axi <command> --help` for per-command usage.
