...
The gate runs when a PR is opened, edited, synchronized, or reopened, including when commits are pushed.
That is safe because no-mistakes writes the attestation before it pushes (no-mistakes #994); use `git push no-mistakes` so the body is rewritten for the new head.
